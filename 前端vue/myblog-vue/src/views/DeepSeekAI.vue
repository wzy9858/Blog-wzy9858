<template>
  <div>
    <el-button type="primary" @click="f1 = !f1" style="position: fixed; top: 110px; right: 100px;">
      打开/关闭 AI
    </el-button>
    ...
    <div class="ai-stream-container" v-if="f1">
      ...
      <div class="input-group">
        <div class="circle-checkbox-container" title="勾选后将引用文章内容">
          <label class="circle-checkbox">
            <input type="checkbox" v-model="isArticleReferenced" />
            <span></span>
          </label>
          <!-- <span class="checkbox-label">引用文章内容</span> -->
        </div>
        <textarea
          v-model="promptText"
          placeholder="请输入你的问题"
...
const isArticleReferenced = ref(false); // 是否勾选引用文章内容
...
const startStream = async () => {
  ...
  if(isArticleReferenced.value){//如果勾选了
    promptText.value = "根据以下我提供的数据回答我的问题\n"+textStore.longText+"\n 下面是问题\n";
  }
  ...
  try {
    ...
    const referenceFlag = isArticleReferenced.value ? '&reference=true' : '';
    eventSource = new EventSource(`http://localhost:8080/ai-stream?prompt=${encodedPrompt}${referenceFlag}`);
    // eventSource = new EventSource(`http://123.56.28.80:8080/ai-stream?prompt=${encodedPrompt}${referenceFlag}`);
